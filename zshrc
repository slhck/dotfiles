...
export TERM EDITOR PAGER DISPLAY LS_COLORS COLORTERM PATH HISTFILE HISTSIZE SAVEHIST RIPGREP_CONFIG_PATH
...
# Agents
alias cx='codex --sandbox danger-full-access --ask-for-approval on-request --add-dir "$PWD/.git" --add-dir "$HOME/Library/Caches/Yarn" --add-dir "$HOME/.yarn" -c sandbox_workspace_write.network_access=true -c approvals_reviewer=auto_review'
